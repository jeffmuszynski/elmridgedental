...
const relatedDescriptions = {
  ...
  '/gum-health-killeen-tx': 'Understand bleeding gums, gingivitis, periodontitis, deep cleanings, and periodontal maintenance.',
...
function serviceBody(page) {
  ...
  const approachHeading = page.approachHeading || 'How Elm Ridge Approaches It';
  ...
  const expectHtml = page.hideExpectSection ? '' : `
    <h2>What to Expect</h2>
    <p>${page.expect}</p>`;
  const callHtml = page.hideCallSection ? '' : `
    <h2>When to Call</h2>
    <p>${page.call}</p>`;
  const paymentHtml = page.hidePaymentSection ? '' : `
  ...
  const optionalSectionHtml = [expectHtml, callHtml, paymentHtml, page.extra || '']
    .filter(Boolean)
  ...
  const lowerSectionHtml = [
    ...
    '<p class="bg-stone border border-teal-light p-5' + (page.readyBlockExtraClass ? ' ' + page.readyBlockExtraClass : '') + '"><strong>Ready for the next step?</strong> Call <a href="' + phoneHref + '">' + phoneDisplay + '</a> or <a href="/request-appointment">request an appointment</a>. For urgent dental problems, call instead of using the form.</p>',
  ].filter(Boolean).join('\n    ');
...
function reviewsHero() {
  return `<section class="bg-charcoal text-white py-20"><div class="max-w-6xl mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center"><div><nav class="text-xs uppercase tracking-widest text-teal mb-6"><a href="/">Home</a></nav><p class="text-xs uppercase tracking-widest text-teal mb-4">Patient Reviews</p><h1 class="font-display text-5xl md:text-6xl font-light leading-tight mb-6">Real reviews from real patients</h1><p class="text-white/70 leading-8 text-lg max-w-3xl">${reviewPhrase}</p><div class="flex flex-col sm:flex-row gap-4 mt-8"><a href="/request-appointment" class="inline-block bg-teal px-8 py-4 text-xs uppercase tracking-widest font-semibold text-center">Request Appointment</a><a href="${phoneHref}" class="inline-block border border-teal px-8 py-4 text-xs uppercase tracking-widest font-semibold text-teal text-center">Call ${phoneDisplay}</a></div></div><div class="flex justify-start lg:justify-center" style="min-height:190px;" aria-label="Live patient reviews from Birdeye"><script type="text/javascript" src="https://birdeye.com/embed/v7/165074186297766/6/123456804131" async></script><div id="bf-revz-widget-123456804131"></div></div></div></section>`;
}
...
const stopBangItems = [
  { label: 'Snoring', text: 'Do you snore loudly enough that others notice?', attrs: 'data-stop="true"' },
  { label: 'Tired', text: 'Do you often feel tired, fatigued, or sleepy during the day?', attrs: 'data-stop="true"' },
  { label: 'Observed', text: 'Has anyone observed you stop breathing, choke, or gasp during sleep?', attrs: 'data-stop="true"' },
  { label: 'Pressure', text: 'Do you have or are you treated for high blood pressure?', attrs: 'data-stop="true"' },
  { label: 'BMI', text: 'Is your BMI over 35 kg/m2?', attrs: 'data-risk-factor="bmi"' },
  { label: 'Age', text: 'Are you over 50?', attrs: '' },
  { label: 'Neck', text: 'Is your neck circumference 16 inches / 40 cm or larger?', attrs: 'data-risk-factor="neck"' },
  { label: 'Gender', text: 'Are you male?', attrs: 'data-risk-factor="male"' },
];

function stopBangQuestionnaireHtml() {
  return `<div id="stop-bang-tool" class="not-prose border border-teal-light bg-stone p-6 my-6"><p class="text-charcoal/65 leading-7 mb-5">Check each item that sounds like you. This screening tool does not diagnose sleep apnea, but it can help you decide whether a take-home sleep study is worth discussing.</p><div class="grid sm:grid-cols-2 gap-3">${stopBangItems.map((item) => `<label class="stop-card flex items-start gap-3 bg-white border border-teal-light p-4 text-sm leading-6"><input type="checkbox" aria-label="${item.label}" class="mt-1" ${item.attrs} /><span><strong>${item.label}:</strong> ${item.text}</span></label>`).join('')}</div><div id="stop-bang-result" class="mt-5 border border-teal-light bg-white p-5 text-charcoal leading-7" role="status" aria-live="polite">Select any statements that apply. Your risk assessment will update here.</div></div><script>(function(){const tool=document.getElementById('stop-bang-tool');if(!tool)return;const boxes=Array.from(tool.querySelectorAll('input[type="checkbox"]'));const result=document.getElementById('stop-bang-result');function checked(selector){return boxes.some(function(box){return box.matches(selector)&&box.checked;});}function update(){const total=boxes.filter(function(box){return box.checked;}).length;const stopCount=boxes.filter(function(box){return box.dataset.stop==='true'&&box.checked;}).length;const highByCombination=stopCount>=2&&(checked('[data-risk-factor="male"]')||checked('[data-risk-factor="bmi"]')||checked('[data-risk-factor="neck"]'));let label='OSA - Low Risk';let detail='Yes to 0 - 2 questions.';if(total>=5||highByCombination){label='OSA - High Risk';detail='Yes to 5 - 8 questions, or yes to 2 or more STOP questions plus male gender, BMI over 35 kg/m2, or neck circumference 16 inches / 40 cm or larger.';}else if(total>=3){label='OSA - Intermediate Risk';detail='Yes to 3 - 4 questions.';}if(result)result.textContent='You answered yes to '+total+' of 8 questions. '+label+': '+detail;}boxes.forEach(function(box){box.addEventListener('change',update);});update();})();</script>`;
}

const sleepApneaIntroHtml = `<section class="py-14 bg-white"><div class="max-w-4xl mx-auto px-6 prose-page space-y-5"><p>You might consider your or your loved one's snoring and sleep disturbances as a common, minor inconvenience, but they can be symptoms of obstructive sleep apnea (OSA).</p><p>An estimated 34-37% of adults have obstructive sleep apnea; however, the vast majority, as many as 80% of those affected, are completely unaware they have it because the condition remains heavily underdiagnosed. <a href="https://www.ama-assn.org/public-health/prevention-wellness/what-doctors-wish-patients-knew-about-sleep-apnea" target="_blank" rel="noopener noreferrer">American Medical Association</a></p></div></section><section class="py-14 bg-stone"><div class="max-w-5xl mx-auto px-6 prose-page space-y-7"><h2>What is Obstructive Sleep Apnea?</h2><p>Obstructive sleep apnea happens when the upper airway repeatedly narrows or collapses during sleep. Breathing can pause or become shallow, oxygen levels may drop, and the brain may briefly wake the body to reopen the airway.</p><p>Common signs include loud snoring, choking or gasping during sleep, restless/poor quality sleep, morning headaches, clenching and grinding teeth, dry mouth, daytime sleepiness, and trouble concentrating. Untreated sleep apnea can strain the heart and body over time and is associated with high blood pressure, heart rhythm problems, heart disease, stroke risk, diabetes, sexual dysfunction, mood changes, and drowsy driving. Early recognition and care are essential for both health and quality of life.</p><figure class="not-prose bg-white border border-teal-light p-3 shadow-xl"><img src="/gbp/sleep-apnea-airway-killeen-tx.png" alt="Airway comparison with obstructive sleep apnea and oral appliance therapy" class="w-full object-cover" loading="lazy" decoding="async"${imageAttrs('gbp/sleep-apnea-airway-killeen-tx.png')} /><figcaption class="grid sm:grid-cols-2 gap-4 mt-3 text-sm text-charcoal/65 leading-6"><span>Obstructive sleep apnea can happen when soft tissue narrows or blocks the airway during sleep. (left image)</span><span>Airway opened with custom sleep apnea oral appliance</span></figcaption></figure></div></section>`;

const sleepApneaTreatmentHtml = `<div class="not-prose grid lg:grid-cols-[1fr,0.9fr] gap-8 items-center my-4"><div class="prose-page space-y-5"><p>Elm Ridge offers custom oral appliance therapy as a comfortable, non-invasive alternative to CPAP therapy. These FDA-cleared devices help keep the airway open, improving breathing and sleep quality without bulky machines or invasive procedures.</p></div><figure class="bg-stone border border-teal-light p-3"><img src="/gbp/mandibular-advancement-device-for-sleep-apnea-killeen-tx-sleep-better.png" alt="Custom mandibular advancement oral appliance for sleep apnea" class="w-full object-cover" loading="lazy" decoding="async"${imageAttrs('gbp/mandibular-advancement-device-for-sleep-apnea-killeen-tx-sleep-better.png')} /></figure></div>`;
...
const serviceEnhancements = {
  ...
  'full-arch-dental-implants-killeen-tx': {
    ...
    answer: 'If you are tired of failing teeth, wearing removable dentures or loose dentures that rely on adhesive, or patchwork dentistry that keeps breaking down, full-arch dental implants are the fixed (non-removable) option designed to rebuild a whole upper or lower arch. The goal is teeth that look natural, feel secure, let you chew with confidence, and are built for long-term durability.',
    image: 'gbp/all-on-4-implants-before-and-after.jpg',
    alt: 'Before-and-after full-arch dental implant result at Elm Ridge',
    imageCaption: 'Individual results vary. Images are shared with patient consent.',
    afterIntroBlockHtml: '<section class="pb-10 bg-white"><div class="max-w-5xl mx-auto px-6 grid md:grid-cols-2 gap-5"><figure class="border border-teal-light bg-stone p-2 shadow-xl"><img src="/gbp/full-arch-implant-denture-all-on-4-killeen-tx.png" alt="Full-arch implant denture for All-on-4-style treatment" class="w-full" loading="lazy" decoding="async" width="889" height="987" /></figure><figure class="border border-teal-light bg-stone p-2 shadow-xl"><img src="/gbp/full-arch-implants.png" alt="Full-arch dental implants supporting fixed implant teeth" class="w-full" loading="lazy" decoding="async" width="1672" height="941" /></figure></div></section>',
    glance: [
    ...
    approach: 'Elm Ridge plans full-arch treatment starting with the end result we desire and working backwards - ensuring the smile, bite, chewing function, speech, cleaning access, facial support, and final material are planned from the start.',
    detailSections: [
      { title: 'Full-Arch Implants are more than just a denture upgrade', html: '<p>Full arch implants are the premium fixed-tooth (non-removable) replacement option for patients who need to replace an entire upper or lower arch and want the closest practical alternative to strong natural teeth. The goal result is a stable, esthetic, comfortable, functional, and durable smile you can be confident in and depend on.</p>' },
      ...
      { title: 'Workflow and timeline', html: htmlList(['Consultation and imaging.', 'Discussion of goals, smile, bite, bone, health history, finances, and final tooth design.', 'Extractions and implant placement when appropriate.', 'Immediate temporary fixed teeth the same day or next day when possible. Temporary removable healing denture when a fixed-temporary is not possible. You\'ll never go without teeth.', 'Healing phase.', 'Final zirconia teeth after healing, commonly around 5-7 months. Timelines vary.']) },
      { title: 'Full-Arch Fixed Implants vs Snap-On Dentures', html: simpleTable(['Option', 'What to know'], [['Full-Arch Fixed Implants', 'Teeth do not come out. They are removable only by a dentist, feel stronger for chewing, and are the best fit when the patient wants maximum stability, comfort, esthetics, and function. They are the premium full-arch replacement option.'], ['Snap-On Dentures', 'Removable denture and usually less expensive than fixed full-arch treatment. "Snap on" to 2-4 implants for improved retention and improved function. Lower arch snap-ons can be a strong middle-ground option; upper arch snap-ons dentures are more case-dependent and usually better treated with a Fixed Full-Arch option.']]) },
  ...
  'dental-crowns-killeen-tx': {
    ...
    image: 'gbp/dental-crowns-killeen-tx-dentist.jpg',
    alt: 'Elm Ridge dentist evaluating a dental crown in Killeen',
    introBlockImage: 'gbp/dental-crown-killeen-tx.png',
    introBlockAlt: 'Dental crown restoration used to protect a damaged tooth',
    introBlockImageWidth: 1448,
    introBlockImageHeight: 1086,
    glance: [
  ...
  'wisdom-teeth-removal-killeen-tx': {
    ...
    whoHeading: 'Why might wisdom teeth need removed?',
    who: 'There&rsquo;s often not enough room for wisdom teeth to come in all the way, making them difficult to clean and likely to become painful, infected, or cause problems with neighboring teeth.',
    approachHeading: 'The Elm Ridge Approach',
    approach: 'The doctors at Elm Ridge will discuss whether your wisdom teeth should be removed and use CBCT 3D X-rays to determine your case&rsquo;s complexity and help plan your procedure safely and smoothly.',
    detailSections: [
  ...
  'dentures-killeen-tx': {
    ...
    image: 'gbp/dentures-killeen-tx.png',
    alt: 'Dentures for replacing missing teeth in Killeen',
    glance: [
  ...
  'dental-bridges-killeen-tx': {
    ...
    detailSections: [
      {
        title: 'Dental Bridge vs Dental Implant',
        html: '<div class="not-prose space-y-5 my-5"><img src="/gbp/dental-implant-vs-bridge-killeen-dentist.png" alt="Dental implant compared with a traditional dental bridge" class="w-full border border-teal-light shadow-lg" loading="lazy" decoding="async" width="1672" height="941" /><div class="grid md:grid-cols-2 gap-4"><article class="border border-teal-light bg-stone p-5"><h3 class="font-display text-2xl text-charcoal mb-3">Traditional Bridge</h3><ul class="list-disc pl-5 space-y-3 text-charcoal/70 leading-7"><li>Can be completed faster without surgery, but it requires reshaping the neighboring teeth and attaching them together with the dental bridge.</li><li>Problems with an individual tooth may require the entire bridge to be redone.</li><li>Bridges need careful cleaning around the edges and under the replacement tooth. Elm Ridge explains the tools and technique so the supporting teeth stay healthy.</li></ul></article><article class="border border-teal-light bg-stone p-5"><h3 class="font-display text-2xl text-charcoal mb-3">Dental Implant</h3><ul class="list-disc pl-5 space-y-3 text-charcoal/70 leading-7"><li>Favored because it does not require reshaping or attaching to adjacent teeth.</li><li>May be a better option than a bridge when neighboring teeth are healthy and do not need crowns. Preserving those teeth can matter over the long term.</li><li>Involves surgery and more time to complete the treatment.</li><li>Initially more expensive than a bridge, but typically lasts longer so implants may be the cheaper option long-term.</li></ul></article></div></div>',
      },
  ...
  'cosmetic-bonding-killeen-tx': {
    ...
    image: 'gbp/composite-bonding-cosmetic-dentistry-3.jpg',
    alt: 'Cosmetic bonding smile result with tooth-colored composite',
    afterIntroBlockHtml: '<section class="pb-10 bg-white"><div class="max-w-5xl mx-auto px-6 grid grid-cols-2 gap-5"><figure class="border border-teal-light bg-stone p-2 shadow-xl"><img src="/gbp/composite-bonding-killeen-tx.jpg" alt="Composite bonding cosmetic dentistry result" class="w-full" loading="lazy" decoding="async" width="481" height="510" /></figure><figure class="border border-teal-light bg-stone p-2 shadow-xl"><img src="/gbp/composite-bonding-killeen-tx-1.jpg" alt="Composite bonding close-up result" class="w-full" loading="lazy" decoding="async" width="2745" height="2641" /></figure></div></section>',
    glance: [
  ...
  'sleep-apnea-dentist-killeen-tx': {
    h1: 'Sleep Apnea Treatment',
    title: 'Sleep Apnea Treatment in Killeen, TX | Elm Ridge',
    description: 'Elm Ridge provides sleep apnea treatment in Killeen with take-home sleep studies, physician diagnosis, and custom oral appliance therapy when appropriate.',
    intro: 'Restoring restful sleep with a CPAP alternative.',
    image: 'gbp/sleep-better-sleep-apnea-killeen-tx.png',
    alt: 'Person sleeping comfortably during sleep apnea treatment',
    hideIntroBlock: true,
    ...
    detailSections: [
      { title: 'How sleep apnea is diagnosed', html: '<p>Elm Ridge offers affordable take-home sleep studies that allow you to obtain a diagnosis with a normal night of sleep in your own bed.</p><p>The recorded data is sent to a sleep physician for an official medical diagnosis. From there we can work with your physician to determine the appropriate course of treatment.</p>' },
      { title: 'How Sleep Apnea is Treated', html: sleepApneaTreatmentHtml },
      { title: 'Determine Sleep Apnea risk with the STOP-BANG questionnaire', html: stopBangQuestionnaireHtml() },
    ],
    payment: costContext,
    nextQuestions: [],
    extra: '',
...
function buildServicesHub() {
  const groups = [
    ['Family & Preventive Dentistry', [
      ...
      ['Gum health', '/gum-health-killeen-tx', 'Gingivitis, periodontitis, deep cleanings, and periodontal maintenance when appropriate.'],
    ]],
    ['Restorative Dentistry', [
...
function buildServicePages() {
  const pages = [
    ...
    makePage({ slug: 'dental-cleanings-killeen-tx', name: 'Dental Cleanings', h1: 'Dental Cleanings That Do More Than Polish Teeth', answer: 'Dental cleanings at Elm Ridge remove buildup, check gum health, and help catch small problems before they become expensive surprises.', related: [{ label: 'Family dentistry', href: '/family-dentist-killeen-tx' }, { label: 'Dental fillings', href: '/dental-fillings-killeen-tx' }, serviceLinks.newPatients, serviceLinks.kayla], image: 'gbp/dental-cleanings.png', alt: 'Dental cleaning appointment with a patient in the chair', medical: false, nextQuestions: [] }),
    makePage({
      slug: 'gum-health-killeen-tx',
      name: 'Gum Health and Deep Cleanings',
      title: 'Gum Health and Deep Cleanings in Killeen, TX | Elm Ridge',
      description: 'Learn about gingivitis, periodontitis, scaling and root planing, and periodontal maintenance at Elm Ridge in Killeen.',
      h1: 'Gum Health, Deep Cleanings, and Periodontal Maintenance',
      intro: 'Bleeding gums, tartar below the gumline, and deeper pockets are signs that a regular cleaning may not be enough. Elm Ridge checks gum health carefully and explains when scaling and root planing, often called a deep cleaning, makes sense.',
      answer: 'Gum health starts with diagnosis. Gingivitis is early gum inflammation that can often improve with better cleaning and home care. Periodontitis is more serious because bacteria and tartar affect the deeper gum attachment and supporting bone.',
      glance: [['Gingivitis', 'Early gum inflammation, often with bleeding or tenderness.'], ['Periodontitis', 'Deeper infection that can affect gum attachment and supporting bone.'], ['Deep cleaning', 'Scaling and root planing removes buildup from below the gumline when a routine cleaning is not enough.'], ['Maintenance', 'Periodontal maintenance is commonly needed every 3-4 months after deep cleaning.']],
      who: 'This page helps patients with bleeding gums, bad breath, tartar buildup, gum pockets, bone loss, or a recommendation for scaling and root planing.',
      approach: 'Elm Ridge measures gum pockets, checks bleeding, reviews X-rays for bone support, and explains whether the issue looks like gingivitis, periodontitis, or another gum-related problem.',
      detailSections: [
        { title: 'Gingivitis vs periodontitis', html: '<p>Gingivitis is inflammation at the gumline. It may cause bleeding, redness, swelling, or tenderness, but it has not yet caused the deeper attachment and bone changes seen with periodontitis.</p><p>Periodontitis is gum disease that reaches deeper around the teeth and creates pockets where bacteria and tartar collect below the gumline. This bacteria attacks the gum attachment and causes irreversible bone loss around the teeth, eventually causing so much damage the teeth become loose and need removed. Early diagnosis and treatment is key to minimize damage.  That is why a regular polishing visit may not be enough once deeper pockets and buildup are present.</p>' },
        { title: 'When scaling and root planing is needed', html: '<p>A routine cleaning removes buildup above the gumline and just below the edge of the gums. Scaling and root planing is recommended when tartar, bacteria, bleeding, and pocketing extend deeper below the gumline. Patients often hear this called a deep cleaning. The goal is to remove the buildup that a regular cleaning cannot safely reach and help the gums heal as much as possible.</p>' },
        { title: 'Why maintenance cleaning may be needed every 3-4 months', html: '<p>After scaling and root planing, periodontal maintenance is usually recommended every 3-4 months to prevent relapse back into a state of active periodontal-disease. Patients with periodontitis often have more prevalance of perio-disease-causing bacteria and deeper pockets or harder to clean areas that can harbor the disease. These visits help disrupt bacteria before it builds back into deeper pockets, keep root surfaces clean and smooth, monitor gum stability, and reduce the chance that the disease keeps progressing quietly.</p>' },
        { title: 'When a referral may be needed', html: '<p>Elm Ridge provides routine periodontal therapy and maintenance when appropriate. Advanced periodontal disease, severe bone loss, or cases needing specialist-level gum treatment may be referred to a periodontist.</p>' },
      ],
      expect: 'Expect gum measurements, appropriate X-rays, a review of bleeding and pocket depths, and a clear explanation of whether a regular cleaning, deep cleaning, maintenance, or referral is the right next step.',
      call: 'Call if your gums bleed when brushing or flossing, your gums feel swollen or tender, you have persistent bad breath, you were told you need a deep cleaning, or you have not had a gum evaluation in a while.',
      providers: ['Jeff Muszynski, DDS', 'Kayla Muszynski, DDS'],
      related: [{ label: 'Dental cleanings', href: '/dental-cleanings-killeen-tx' }, { label: 'Family dentistry', href: '/family-dentist-killeen-tx' }, { label: 'New patients', href: '/new-patients' }, serviceLinks.kayla],
      image: 'gbp/deep-cleanings-gum-health-periodontitis-killeen-tx.png',
      alt: 'Deep cleaning and gum health treatment for periodontitis in Killeen',
      medical: true,
      serviceType: 'Periodontal therapy',
      procedureType: 'Scaling and root planing',
      bodyLocation: 'Gums',
      faq: [
        ['Is a deep cleaning the same as a regular cleaning?', 'No. A regular cleaning is for routine buildup. Scaling and root planing, often called a deep cleaning, is used when buildup and inflammation extend below the gumline.'],
        ['Why do I need periodontal maintenance every 3-4 months?', 'After deep cleaning, bacteria can rebuild in deeper pockets faster than a six-month routine schedule can control. Maintenance every 3-4 months helps monitor and manage gum disease over time.'],
        ['Can gum disease be cured?', 'Gingivitis can often improve when plaque and tartar are controlled. Periodontitis is usually managed long term with deep cleaning, maintenance visits, home care, and referral when disease is advanced.'],
      ],
    }),
    makePage({ slug: 'dental-fillings-killeen-tx', name: 'Dental Fillings', h1: 'Tooth-Colored Fillings for Cavities and Small Breaks', answer: 'Dental fillings repair small to moderate areas of decay or damage. Elm Ridge uses tooth-colored materials and explains when a filling is enough versus when a crown is the better long-term answer.', related: [{ label: 'Dental crowns', href: '/dental-crowns-killeen-tx' }, { label: 'Broken tooth', href: '/broken-tooth-killeen-tx' }, serviceLinks.kayla], image: 'gbp/dental-fillings-killeen-dentist.png', alt: 'Tooth-colored dental filling replacing an old metal filling' }),
    ...
    makePage({ slug: 'single-tooth-implant-killeen-tx', name: 'Single Tooth Implant', h1: 'Replacing One Missing Tooth With an Implant', answer: 'A single tooth implant can replace one missing tooth without reshaping healthy neighboring teeth, when bone, gum health, bite, and timing support the plan.', providers: ['Jeff Muszynski, DDS'], related: implantRelated, image: 'gbp/single-tooth-dental-implant.png', alt: 'Single tooth dental implant with implant crown' }),
    makePage({ slug: 'implant-bridge-killeen-tx', name: 'Implant Bridge', h1: 'Implant Bridges for Multiple Missing Teeth', answer: 'An implant bridge can replace multiple missing teeth with fewer implants than one implant per tooth. Elm Ridge plans both the surgical and restorative parts of the case.', providers: ['Jeff Muszynski, DDS'], related: implantRelated, image: 'gbp/implant-bridge.png', alt: 'Implant bridge supported by dental implants', faq: [['Can an implant bridge replace several teeth?', 'Yes. It can replace multiple teeth in a row when the implants and bite can support the design.'], ['Can Elm Ridge restore implants placed elsewhere?', 'Yes, after evaluation and with records, system information, and imaging as needed.']] }),
    makePage({ slug: 'full-arch-dental-implants-killeen-tx', name: 'Full-Arch Dental Implants', h1: 'Full-Arch Dental Implants', intro: 'Full-arch dental implants are for patients who need to replace an entire upper or lower arch and want the strongest, most natural, comfortable, esthetic, functional, and durable option they can choose. Often searched as All-on-4, All-on-X, hybrid dentures or fixed implant teeth, this treatment is planned around your bone, bite, face, smile, speech for a long-term and beautiful result.', answer: 'If you are tired of failing teeth, wearing removable dentures or loose dentures that rely on adhesive, or patchwork dentistry that keeps breaking down, full-arch dental implants are the fixed (non-removable) option designed to rebuild a whole upper or lower arch. The goal is teeth that look natural, feel secure, let you chew with confidence, and are built for long-term durability.', providers: ['Jeff Muszynski, DDS'], related: implantRelated, image: 'gbp/all-on-4-implants-before-and-after.jpg', alt: 'Before-and-after full-arch dental implant result at Elm Ridge', imageCaption: 'Individual results vary. Images are shared with patient consent.', faq: [['Do you provide both surgery and restoration?', 'Yes. Elm Ridge provides surgery and restorative treatment for full-arch cases.'], ['What final material do you usually use?', 'Zirconia is the go-to final material. Acrylic is reserved for rare or selected situations when appropriate.'], ['Can you restore implants placed elsewhere?', 'Sometimes, after evaluation and with records, system details, and imaging as needed.']] }),
    ...
    makePage({ slug: 'cosmetic-bonding-killeen-tx', name: 'Cosmetic Bonding', h1: 'Cosmetic Bonding', answer: 'Cosmetic bonding can repair small chips, close selected small gaps, and improve tooth shape with tooth-colored composite. It is often more conservative than veneers but not right for every case.', related: cosmeticRelated, providers: ['Kayla Muszynski, DDS'], image: 'gbp/composite-bonding-cosmetic-dentistry-3.jpg', alt: 'Cosmetic bonding smile result with tooth-colored composite', afterIntroBlockHtml: '<section class="pb-10 bg-white"><div class="max-w-5xl mx-auto px-6 grid grid-cols-2 gap-5"><figure class="border border-teal-light bg-stone p-2 shadow-xl"><img src="/gbp/composite-bonding-killeen-tx.jpg" alt="Composite bonding cosmetic dentistry result" class="w-full" loading="lazy" decoding="async" width="481" height="510" /></figure><figure class="border border-teal-light bg-stone p-2 shadow-xl"><img src="/gbp/composite-bonding-killeen-tx-1.jpg" alt="Composite bonding close-up result" class="w-full" loading="lazy" decoding="async" width="2745" height="2641" /></figure></div></section>' }),
