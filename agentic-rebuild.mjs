...
function reviewsHero() {
  return `<section class="bg-charcoal text-white py-20"><div class="max-w-6xl mx-auto px-6"><nav class="text-xs uppercase tracking-widest text-teal mb-6"><a href="/">Home</a></nav><p class="text-xs uppercase tracking-widest text-teal mb-4">Patient Reviews</p><h1 class="font-display text-5xl md:text-6xl font-light leading-tight mb-6">Real reviews from real patients</h1><p class="text-white/70 leading-8 text-lg max-w-3xl">${reviewPhrase}</p><div class="flex flex-col sm:flex-row gap-4 mt-8"><a href="/request-appointment" class="inline-block bg-teal px-8 py-4 text-xs uppercase tracking-widest font-semibold text-center">Request Appointment</a><a href="${phoneHref}" class="inline-block border border-teal px-8 py-4 text-xs uppercase tracking-widest font-semibold text-teal text-center">Call ${phoneDisplay}</a></div><div class="mt-10 bg-white text-charcoal border border-teal-light p-4 sm:p-6 shadow-xl" style="min-height:260px;" aria-label="Live patient reviews from Birdeye"><script type="text/javascript" src="https://birdeye.com/embed/v7/165074186297766/6/123456804131" async></script><div id="bf-revz-widget-123456804131"></div></div></div></section>`;
}
...
const sleepApneaTreatmentHtml = `<div class="not-prose grid lg:grid-cols-[1fr,0.9fr] gap-8 items-center my-4"><div class="prose-page space-y-5"><p>Elm Ridge offers custom oral appliance therapy as a comfortable, non-invasive alternative to CPAP therapy. These FDA-cleared devices help keep the airway open, improving breathing and sleep quality without bulky machines or invasive procedures.</p></div><figure class="bg-stone border border-teal-light p-3"><img src="/gbp/mandibular-advancement-device-for-sleep-apnea.png" alt="Custom mandibular advancement oral appliance for sleep apnea" class="w-full object-cover" loading="lazy" decoding="async"${imageAttrs('gbp/mandibular-advancement-device-for-sleep-apnea.png')} /></figure></div>`;
