...
const noindexPaths = new Set(['/ai-summary', '/post-op/deep-cleaning', '/accessibility-statement']);
...
export function head(title, description, path) {
  ...
  const robots = noindexPaths.has(path) ? 'noindex, follow' : 'index, follow';
...
export const menuScript = `<script>const menuBtn=document.getElementById('menu-btn'),mobileMenu=document.getElementById('mobile-menu'),iconMenu=document.getElementById('icon-menu'),iconClose=document.getElementById('icon-close');if(menuBtn&&mobileMenu){const syncMenu=()=>{const isOpen=!mobileMenu.classList.contains('hidden');menuBtn.setAttribute('aria-expanded',String(isOpen));menuBtn.setAttribute('aria-label',isOpen?'Close menu':'Open menu');mobileMenu.setAttribute('aria-hidden',String(!isOpen));iconMenu&&iconMenu.classList.toggle('hidden',isOpen);iconClose&&iconClose.classList.toggle('hidden',!isOpen)};const closeMenu=()=>{mobileMenu.classList.add('hidden');mobileMenu.classList.remove('flex');syncMenu()};menuBtn.addEventListener('click',()=>{mobileMenu.classList.toggle('hidden');mobileMenu.classList.toggle('flex');syncMenu()});mobileMenu.querySelectorAll('a').forEach(a=>a.addEventListener('click',closeMenu));document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!mobileMenu.classList.contains('hidden')){closeMenu();menuBtn.focus()}});syncMenu();}</script>`;
...
export function writePage(file, opts) {
  ...
  const faqHtml = faqItems.length ? `<section class="py-16 bg-stone"><div class="max-w-4xl mx-auto px-6"><h2 class="font-display text-4xl mb-8">${faqHeading}</h2><div class="space-y-4">${faqItems.map(({ question, answerHtml }, index)=>{ const panelId = `faq-panel-${index + 1}`; return `<div class="bg-white border border-teal-light p-6"><button type="button" class="faq-button font-semibold" aria-expanded="false" aria-controls="${panelId}">${question}</button><div id="${panelId}" class="faq-panel" hidden><p class="mt-3 text-charcoal/65 leading-7">${answerHtml}</p></div></div>`; }).join('')}</div></div></section>` : '';
